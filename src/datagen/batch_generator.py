...
from datetime import date, datetime, timezone  # For timestamping and date partitioning
...
from ecommerce_generator import EcommerceDataGenerator  # noqa: E402
...
class S3BatchUploader:
    ...
    def upload_jsonl(self, records: list, entity: str, filename: str) -> str:
...
class NexusFlowBatchGenerator:
    ...
    def __init__(
        ...
        logger.info(
            f"Initializing generator — mode: {mode}, "
            f"customers: {num_customers:,}, seed: {seed}"
        )

        # Core data generator
        self.gen = EcommerceDataGenerator(num_customers=num_customers, seed=seed)

        # S3 uploader (only in s3 mode)
        if mode == "s3":
            self.uploader = S3BatchUploader(bucket=s3_bucket, region=aws_region)
        else:
            os.makedirs(output_dir, exist_ok=True)
            os.makedirs(
                f"{output_dir}/bronze/customers/date={self.today}", exist_ok=True
            )
            os.makedirs(f"{output_dir}/bronze/orders/date={self.today}", exist_ok=True)
            os.makedirs(
                f"{output_dir}/bronze/inventory/date={self.today}", exist_ok=True
            )
            os.makedirs(f"{output_dir}/bronze/reviews/date={self.today}", exist_ok=True)
    ...
    def run_customers(self) -> None:
        ...
        if self.mode == "s3":
            self.uploader.upload_jsonl(
                records=records, entity="customers", filename=filename
            )
        else:
    ...
    def run_orders(self, num_orders: int = 100_000) -> None:
        ...
        for i, order in enumerate(
            ...
            if len(buffer) >= chunk_size:
                ...
                if self.mode == "s3":
                    self.uploader.upload_jsonl(
                        records=buffer, entity="orders", filename=filename
                    )
                else:
                    path = (
                        f"{self.output_dir}/bronze/orders/"
        ...
        if buffer:
            ...
            if self.mode == "s3":
                ...
                path = (
                    f"{self.output_dir}/bronze/orders/" f"date={self.today}/{filename}"
                )
                with open(path, "w") as f:
                    for r in buffer:
                        f.write(json.dumps(r) + "\n")
        ...
        logger.info(
            f"  ✅ Orders complete: {num_orders:,} records " f"in {chunk_num} chunks"
        )
    ...
    def run_inventory(self) -> None:
        ...
        if self.mode == "s3":
            ...
            path = (
                f"{self.output_dir}/bronze/inventory/" f"date={self.today}/{filename}"
            )
    ...
    def run_reviews(self, num_reviews: int = 10_000) -> None:
        ...
        if self.mode == "s3":
            ...
            path = f"{self.output_dir}/bronze/reviews/" f"date={self.today}/{filename}"
            with open(path, "w") as f:
                f.write(xml_content)
            logger.info(f"  ✅ Written: {path}")

    def run_all(self, num_orders: int = 100_000, num_reviews: int = 10_000) -> None:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="NexusFlow Batch Data Generator")
    parser.add_argument(
        "--mode",
        choices=["local", "s3"],
        default="local",
