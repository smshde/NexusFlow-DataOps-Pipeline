...
from dataclasses import asdict, dataclass  # For data modeling
from datetime import datetime, timedelta, timezone  # For date handling
from typing import Generator, Optional  # For type hints and generator functions
...
UTM_SOURCES = [
    ...
    "youtube",
    "affiliate",
]  # Urchin Tracking Module to track where traffic is coming from
...
UTM_MEDIUMS = [
    ...
    "referral",
    "none",
]  # what type of channel is sending the traffic? Cost Per Click, Organic Search, Social Media, Email, Referral, None
    ...
    def generate(cls) -> "Customer":
        ...
        return cls(
            ...
            segment=random.choice(
                ["bargain_hunter", "loyal", "at_risk", "new", "vip", "dormant"]
            ),
        )
...
class Order:
    ...
    @classmethod
    def generate(
        cls, customer_id: str, order_date: Optional[datetime] = None
    ) -> "Order":
        ...
        for p in products:
            ...
            discount = round(
                p["price"] * random.choice([0, 0, 0, 0.05, 0.10, 0.15, 0.20]), 2
            )
            items.append(
                OrderItem(
        ...
        return cls(
            ...
            actual_delivery_date=(
                actual_delivery.date().isoformat() if actual_delivery else None
            ),
...
class ClickstreamEvent:
    ...
    referrer_url: Optional[
        str
    ]  # Attribution: From where did the user come to this page?
    ...
    scroll_depth_pct: Optional[
        int
    ]  # Percentage of page scrolled (for page_view events/engagement)
    time_on_page_sec: Optional[
        int
    ]  # Time spent on page in seconds (for page_view events)
    utm_source: Optional[str]
    utm_medium: Optional[str]
    _ingestion_ts: (
        str  # Timestamp when event was generated (for data freshness and ordering)
    )
    ...
    def generate(
        ...
        product = (
            random.choice(PRODUCT_CATALOG)
            if event_type
            in ["product_view", "add_to_cart", "remove_from_cart", "review_view"]
            else None
        )
        ...
        return cls(
            ...
            scroll_depth_pct=(
                random.randint(10, 100) if event_type == "page_view" else None
            ),
            time_on_page_sec=(
                random.randint(5, 600) if event_type == "page_view" else None
            ),
...
class EcommerceDataGenerator:
    ...
    def generate_clickstream_batch(self, num_events: int = 500_000) -> Generator:
        ...
        for _ in range(
            num_events
        ):  # sessions will have multiple events, so we generate session-level data and then events within that session
            ...
            for j in range(
                events_in_session
            ):  # Generate multiple events for the same session, with timestamps incrementing by a few seconds to simulate user behavior within a session
    ...
    def generate_reviews_xml(self, num_reviews: int = 10_000) -> str:
        ...
        root = ET.Element(
            "reviews", {"generated_at": datetime.now(timezone.utc).isoformat()}
        )
        ...
        for _ in range(num_reviews):
            ...
            ET.SubElement(review, "verified_purchase").text = str(
                random.random() > 0.2
            ).lower()
...
if __name__ == "__main__":
    ...
    if args.mode in ("batch", "all"):
        ...
        with open(f"{args.output_dir}/reviews.xml", "w") as f:
            f.write(reviews_xml)
        print("✅ Reviews XML written")

        if s3_bronze_bucket:
            s3 = boto3.client("s3")
            s3.upload_file(
                customers_file, s3_bronze_bucket, "bronze/customers/customers.jsonl"
            )
            print(
                f"✅ Uploaded customers.jsonl to s3://{s3_bronze_bucket}/bronze/customers/"
            )
            s3.upload_file(
                inventory_file,
                s3_bronze_bucket,
                f"bronze/inventory/{snapshot_date}/inventory.csv",
            )
            print(
                f"✅ Uploaded inventory.csv to s3://{s3_bronze_bucket}/bronze/inventory/{snapshot_date}/"
            )
        else:
            print(
                "⚠️  S3_BRONZE_BUCKET not set — customers/inventory stay local only, bronze_to_silver will find nothing"
            )
