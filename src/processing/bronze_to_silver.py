...
from pyspark.sql import (  # For Spark session and window functions
    DataFrame,
    SparkSession,
    Window,
)
...
def mask_email(email: str) -> str:
    ...
    local = parts[0]
    masked = (
        local[0] + "***" + (local[-1] if len(local) > 1 else "")
        if len(local) > 0
        else "***"
    )
...
def clean_reviews_df(df: "DataFrame") -> "DataFrame":
    """Type-cast + validate raw product-review rows (parsed from XML)."""
    cleaned = (
        df.withColumn("rating", F.col("rating").cast("int"))
        .withColumn("helpful_votes", F.col("helpful_votes").cast("int"))
        .withColumn("images_count", F.col("images_count").cast("int"))
        .withColumn("verified_purchase", F.col("verified_purchase") == F.lit("true"))
...
class BronzeToSilverProcessor:
    ...
    def __init__(
        self, spark: SparkSession, env: str, bronze_bucket: str, silver_bucket: str
    ):
        self.spark = spark
        self.env = env
    ...
    def process_orders(self, date_partition: str) -> None:
        ...
        # Read raw JSON
        df_raw = (
            self.spark.read.schema(ORDER_SCHEMA)
            .option(
                "mode", "PERMISSIVE"
            )  # PERMISSIVE — parse best-effort, put bad rows in a special column,DROPMALFORMED — silently discard bad rows,FAILFAST — throw an exception on first bad row
        ...
        df = df_raw.filter(F.col("_corrupt_record").isNull()).drop(
            "_corrupt_record"
        )  # Keep only well-formed records for processing,  drop null records
        ...
        # ── TYPE CASTING ──
        df = (
            df.withColumn("order_date", F.to_timestamp("order_date"))
    ...
    def process_customers(self) -> None:
        ...
        df_raw = (
            self.spark.read.schema(CUSTOMER_SCHEMA)
    ...
    def process_clickstream(self, date_partition: str) -> None:
        ...
        df = (
            self.spark.read.schema(CLICKSTREAM_SCHEMA)
            .option("mode", "PERMISSIVE")
            .json(input_path)
        )
        ...
        df = df.dropDuplicates(
            ["event_id"]
        )  # Assuming event_id is unique, this will remove any duplicate events that may have been ingested multiple times due to retries or duplicates in the source data.
    ...
    def process_inventory(self, snapshot_date: str) -> None:
        ...
        df = (
            ...
            .withColumn(
                "quantity_on_hand", F.col("quantity_on_hand").cast(IntegerType())
            )
            .withColumn(
                "quantity_reserved", F.col("quantity_reserved").cast(IntegerType())
            )
            .withColumn(
                "quantity_available", F.col("quantity_available").cast(IntegerType())
            )
            .withColumn("reorder_point", F.col("reorder_point").cast(IntegerType()))
            .withColumn(
                "reorder_quantity", F.col("reorder_quantity").cast(IntegerType())
            )
            .withColumn("unit_cost", F.col("unit_cost").cast(DoubleType()))
            .withColumn("last_received_date", F.to_date("last_received_date"))
            # Derived
            .withColumn(
                "is_low_stock", F.col("quantity_available") <= F.col("reorder_point")
            )
    ...
    def process_reviews(self, date_partition: str) -> None:
        """Process product reviews XML → partitioned Parquet."""
        input_path = f"{self.bronze_base}/reviews/date={date_partition}/*.xml"
        df = self.spark.read.format("xml").option("rowTag", "review").load(input_path)
        cleaned = clean_reviews_df(df)
        (
            cleaned.write.mode("overwrite")
            .partitionBy("review_year", "review_month")
            .format("parquet")
            .save(f"{self.silver_base}/reviews/")
        )
...
def create_spark_session(app_name: str, aws_region: str) -> SparkSession:
    return (
        ...
        )  # Enable adaptive query execution for better performance on large datasets with skew or varying partition sizes
        .config(
            "spark.sql.adaptive.coalescePartitions.enabled", "true"
        )  # Dynamically coalesce shuffle partitions based on runtime statistics to reduce small files and improve downstream performance
        .config(
            "spark.sql.adaptive.skewJoin.enabled", "true"
        )  # Automatically handle skewed joins by splitting large partitions to avoid OOM errors and improve join performance
        .config("spark.sql.parquet.compression.codec", "snappy")
        .config(
            "spark.sql.shuffle.partitions", "200"
        )  # Set a reasonable default for shuffle partitions; can be tuned based on cluster size and data volume
        .config(
            "spark.hadoop.fs.s3a.impl", "org.apache.hadoop.fs.s3a.S3AFileSystem"
        )  # Use S3A for better performance and support for larger files when reading/writing to S3
        .config(
            "spark.hadoop.fs.s3a.aws.credentials.provider",
            "com.amazonaws.auth.InstanceProfileCredentialsProvider",
        )  # Use IAM roles for authentication when running on AWS (e.g., EMR, EKS with IRSA)
        .config(
            "spark.serializer", "org.apache.spark.serializer.KryoSerializer"
        )  # Use Kryo serialization for better performance when shuffling complex objects
        .config(
            "spark.sql.catalog.spark_catalog",
            "org.apache.spark.sql.delta.catalog.DeltaCatalog",
        )  # for Delta Lake silver layer, configure the catalog to use Delta
        .getOrCreate()
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=["orders", "customers", "clickstream", "inventory", "reviews"],
    )
    ...
    if "reviews" in args.entities:
        processor.process_reviews(args.date_partition)
